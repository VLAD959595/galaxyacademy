...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_text = f"""
🏒 Добро пожаловать в Хоккейную Академию "Ледяные Волки"! 
...
async def schedule(update: Update, context: ContextTypes.DEFAULT_TYPE):
    conn = sqlite3.connect('hockey_academy.db')
    cursor = conn.cursor()
...
async def academy_info(update: Update, context: ContextTypes.DEFAULT_TYPE):
    info_text = """
🏒 <b>Хоккейная Академия "Ледяные Волки"</b>

📍 <b>Адрес:</b> г. Москва, ул. Спортивная, 15
...
📧 <b>Email:</b> info@icewolves.ru
🌐 <b>Сайт:</b> www.icewolves.ru
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get('awaiting') == 'age':
    ...
    elif context.user_data.get('awaiting') == 'skill':
        skill_level = text
        ...
        """, (
            ...
            context.user_data['phone'], context.user_data['age'], skill_level
        ))
